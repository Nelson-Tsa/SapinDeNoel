  ...
  function afficherSapin(etages, hauteur_etage) {
    ...
    for (let j = 0; j < etages ; j++) {
        if(etages%2 == 0) {
            ...
            nouveauSapin.innerHTML += " ".repeat(((etages+hauteur_etage )/ 2) - 1) +"  "+"#".repeat(etages) + "<br>";
            divSapin.appendChild(nouveauSapin)
           
        }
        else {
            nouveauSapin.innerHTML += " ".repeat(etages+hauteur_etage / 2) +"  "+"#".repeat(etages)+ "<br>";
            divSapin.appendChild(nouveauSapin)
...
// //   divSapin.innerHTML = sapin.join('');
  ...
  function afficherEtage(hauteur, pointe_offset, espacement) {
    ...
    if (pointe_offset == 0) {
        nouveauSapin.innerHTML = " ".repeat(espacement) +" ".repeat(hauteur + 1) + "+" + "<br>";
        divSapin.appendChild(nouveauSapin)
        nouveauSapin.innerHTML += " ".repeat(espacement) +" ".repeat(hauteur) + "/|\\" + "<br>";
        divSapin.appendChild(nouveauSapin)
        ...
        for (let i = 0; i < hauteur; i++) {
          ...
          nouveauSapin.innerHTML += " ".repeat(espacement) +" ".repeat(hauteur - i - 1) +"/" +result +"|" +result +"\\" + "<br>";
          divSapin.appendChild(nouveauSapin)
